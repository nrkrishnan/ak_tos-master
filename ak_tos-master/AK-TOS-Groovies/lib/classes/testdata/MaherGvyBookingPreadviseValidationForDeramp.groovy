...
Steps:
    ...
    Add Notice to call the plug-in
        ...
        3. Enter the values as below:
            ...
            Action:             Execute Code
            (Code):             def rpValidator = api.getGroovyClassInstance("MaherGvyBookingPreadviseValidationForDeramp")
...
public class MaherGvyBookingPreadviseValidationForDeramp {
    ...
    public void execute(Object event, Object api)
    {
        ...
        try {
            ...
            // Booking validations will be done only for containers with containerStatus as FCL and LCL
            if(FreightKindEnum.FCL.equals(unit.getUnitFreightKind()) || FreightKindEnum.LCL.equals(unit.getUnitFreightKind())) {

              println("unit freight kind is : " + unit.getUnitFreightKind());
              ...
              println "Booking number is : " +  bkgNumber;
              ...
              } else if(book.getEqboOrderItems() != null) {
                  println("Checking whether any one of the booking order items is hazardous");
                  boolean isHazardous = isHazardous(book);
                  if(isHazardous) {
                    println("Found one or more booking order item is hazardous");
                    throw BizViolation.create(OrdersPropertyKeys.ERRKEY__HAZARDOUS_CONTAINERS_NOT_RECEIVED_FOR_EXPORT_BOOKING, null, bkgNumber);
                  }
              }
            ...
            try {
                ...
                println("Exception occured in MaherGvyBookingPreadviseValidationForDeramp.groovy" + inEx);
    ...
    private boolean isHazardous(Booking inBooking) {
        Set orderItems = inBooking.getEqboOrderItems();
        if (orderItems != null && !orderItems.isEmpty()) {
            for (Object bkgOrderItem : orderItems) {
              EqBaseOrderItem eqboi = (EqBaseOrderItem) bkgOrderItem;
              EquipmentOrderItem eqoi = EquipmentOrderItem.resolveEqoiFromEqboi(eqboi);
              Hazards hazards = eqoi.getEqoiHazards();
              if (hazards != null) {
                if (hazards.getHazardItemsIterator().hasNext()) {
                    return true;
                }
              }
            }
