    ...
    */
	...
	println("ERRORO_WHILE_ATD_ATA ::"+ e)
        ...
        try{
            ...
            GroovyEvent eventObj = event;
            boolean isVesselDeparted = isPreviousPhaseWorkingOrComplete(event);

            if (!"-jms-".equals(doer) && isVesselDeparted) {
                if ("LOAD".equals(type) ) {
                    ...
                    if(currentFacility!=null){
                        ...
                        if(currentFacilityId == "ANK") {
                            emailTo = emailTo+";1tosnewvesank@matson.com";
                        } else if (currentFacilityId == "KDK") {
                            emailTo = emailTo+";KDKMarineGroup@matson.com;1KDKDispatchersGroup@matson.com";
                        } else if(currentFacilityId == "DUT" || currentFacilityId == "KQA") {
                            emailTo = emailTo+";lsevilla@matson.com;cgarcia@matson.com;cparker@matson.com;jbalbarino@matson.com;";
                        }
                    }
    ...
    public boolean isPreviousPhaseWorkingOrComplete(GroovyEvent inEventObj) {

        println(" calling isPreviousPhaseWorkingOrComplete ");
        ...
        if (inEventObj.getEvent() == null) {
            return false;
        }
        if (inEventObj.getPreviousPropertyAsString("VisitPhase") == null) {
            return false;
        }

        if (CarrierVisitPhaseEnum.WORKING.getName().equalsIgnoreCase(inEventObj.getPreviousPropertyAsString("VisitPhase")) ||
                CarrierVisitPhaseEnum.COMPLETE.getName().equalsIgnoreCase(inEventObj.getPreviousPropertyAsString("VisitPhase"))) {

            if (CarrierVisitPhaseEnum.DEPARTED.getName().equalsIgnoreCase(inEventObj.getPropertyAsString("VisitPhase"))) {
