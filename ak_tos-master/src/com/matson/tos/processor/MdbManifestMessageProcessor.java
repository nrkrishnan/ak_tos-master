...
* A1     10/16/09   GR          Removed the TimeZone Conversion,
*                               Hard Coded HON for VV from port Lookup
* A2     12/10/09   GR          Changed Ports Extraction methods
*                               POD,POL,OPL
* A3     12/10/09   GR          Is client vessel then Don't Process
* A4     12/10/09   GR          Connection Error Mail Notification and AG Response/Arrival Email
*                               Add Vesoy to mail Subject.
* A5     12/10/09   GR          Add Temp Check
* A6     01/21/09	GR          Vinsight Code Change 
*                               1. Moved all validation code above 
*                               2. Send Soap Message Request 
* A7     03/03/10   GR          Limit size to 20 For ShipperRef 
* A8     11/18/10   GR          Error Email to the AG DEPT 
* A9     06/08/11   GR          Added code to handle Routing Error 
* A10    09/14/11   GR          Ignore GuamNewves [Dont Process GuamNewves]
*************************************************************************************
*/
...
/**
 * 
 * @author AZA This class ia a message EJB and handles the unit message in the
 *         topic defined in the ejb-jar.xml file.
 */

public class MdbManifestMessageProcessor implements MessageDrivenBean,
		MessageListener {
	private static Logger logger = Logger
			.getLogger(MdbManifestMessageProcessor.class);
	...
	public MdbManifestMessageProcessor() {
	...
	public void onMessage(Message msg) {
		...
		try {
			...
			//A10 - SKIP GUM NEWVES FOR MANIFEST PROCESSING
			if(text.contains("GuamNewVes")){
				logger.debug("Manifest Proc Skipped GuamNewves");
			}else if(text.contains("NewVes Detention Completed")){
				logger.debug("Manifest Proc Skipped 'NewVes Detention Completed'");
			} else{ //PROC HON NEWVES 
 		       processMsg(text);
			   logger.debug("After processMsg!");
			}
		}
		catch (JMSException ex) {
			logger.error("JMSException: ", ex);
			try {
				ManifestHelper.sendEmail("AG Manifest JMS Exception: \n"+ ex);
				_context.getUserTransaction().commit();
			} catch (Throwable e) {
				logger.error("Error Commiting Transaction Context : " + e);
			}
		}catch (ConnectException e) {
			logger.error("ConnectException: ", e);
			try {
				ManifestHelper.sendEmail(ManifestHelper.EMAIL_ADDR,AG_MANIFEST_ERROR_EMAIL,
				"AG Manifest Service Connection Exception Posting Vessel "+getVesvoy(),REPROCESS_MSG+e);
				_context.getUserTransaction().commit();
			} catch (Throwable ex) {
				logger.error("Error Commiting Transaction Context : " + ex);
			}
			...
			try {
				ManifestHelper.sendEmail(ManifestHelper.EMAIL_ADDR,AG_MANIFEST_ERROR_EMAIL,
				"AG Manifest Failure Message for Vessel "+getVesvoy(),REPROCESS_MSG+e);
				_context.getUserTransaction().commit();
			} catch (Throwable ex) {
				logger.error("Error Commiting Transaction Context : " + ex);
			}
	...
	public ManifestWSRequest processMsg(String msgText) throws Exception {
		...
		//logger.debug("Manifest processMsg called with message : " + msgText);
		logger.debug("Manifest processMsg called");
		try {
	...
	private void callAgriDeptService(ManifestWSRequest req) throws ServiceException,
