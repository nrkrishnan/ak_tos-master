 ...
 * A8 GR  02/23/2011 Removed Logging 
 */

public class AcetsMessageProcessor {
	private static final String PIPE_SPLIT = String.valueOf((char) 0xA6);
	public static final String ALASKA_TRUCKING_COMPANIES = "ALASKA_TRUCKING_COM";
	private static Logger logger = Logger.getLogger(AcetsMessageProcessor.class);
	...
	public void processMsg( String msgText, String source) {
		...
		try {
			if ( msgText.startsWith( "ATRA") || msgText.startsWith( "ATRU") ||
				...
				/* ATR U,A,D is not required for Alaska, no longer need to process them */
				...
				/*Boolean isSkipProcessingATRU = Boolean.TRUE;
				if(atrxMessageHandler == null) {
					if (msgText.startsWith("ATRU")) {
						try {
							...
							if (tempCompanies != null) {
								...
								String snxCompanyId = msgText.split(PIPE_SPLIT)[5];
								if (snxCompanyId != null && companies.contains(snxCompanyId)) {
									// continue the flow
									isSkipProcessingATRU = Boolean.FALSE;
								}
							}
						} catch (Exception e) {
							logger.error("Error in processing an ATRU to snx for company id" + msgText.split(PIPE_SPLIT)[5], e);
						}
					} else {
						isSkipProcessingATRU = Boolean.FALSE;
					}
					if (!isSkipProcessingATRU)
...
//				// process LNK messages
				...
				//String loadPort= ((Alnk)msgHandler.getTextObj()).getLnkLoadPort();
				 //logger.debug( "port1: " + port+" ");
...
//				// process BOB messages. First send out booking update message
				...
				if ( msgHandler != null) {
					...
					if ("MQ".equalsIgnoreCase(source)){
						logger.debug( "XML for MQ :" + xml);
						sender.send( xml);
					}else{
						logger.debug( "XML for JMS :" + xml);
						sender.sendBob(xml);
					}	
				...
				if (msgText != null) {
					...
					if (strings != null && strings.length >= 6) {
						...
						try {
							...
							//psethuraman : making contianerNumber null, to seperate HAZMAT Container webservice with Booking webservice
							postHazardToTOS( _billNumber, null);
						} catch (TosException tex) {
							logger.error("Error:", tex);
							tex.printStackTrace();
							String mailText = "Abdx Message		"+msgText;
							mailText+="\nException is\n"+tex.getMessage();
							mailText+="\nStackTarce\n"+tex.getStackTrace();
							EmailSender.sendMail(supportMail, supportMail, "Exception Processing ABDX Message: " + tex.getMessage(), mailText);
						} catch (Exception ex) {
							...
							mailText+="\nException is\n"+ex.getMessage();
							mailText+="\nStackTarce\n"+ex.getStackTrace();
							EmailSender.sendMail(supportMail, supportMail, "Exception Processing ABDX Message: " + ex.getMessage(), mailText);
						}
					}
				}
			} else if ( msgText.startsWith( "AVGX")) {
//				// process VGX messages. send out vgm weight update message
				AvgxMessageHandler vgxHandler = new AvgxMessageHandler("com.matson.tos.jaxb.snx",
						"com.matson.tos.jatb", "/xml/avgx.xml", AbstractMessageHandler.TEXT_TO_XML);
				if (vgxHandler != null) {
					vgxHandler.setTextStr(msgText);
					xml = vgxHandler.getXmlStr();
					JMSSender sender1 = new JMSSender(JMSSender.REAL_TIME_QUEUE, vgxHandler.getFacilityToPost());
					logger.info("Posting VGX message to : "+vgxHandler.getFacilityToPost());

					if (xml != null) {
						logger.debug("VGX XML for MQ :" + xml);
						sender1.send(xml);
					} else {
						logger.warn("Unit SNX with VGX not sent to TOS, XML is "+xml);
					}
				}
			} else if (msgText.startsWith("CUPD")) {

				logger.debug("calling chassis update " + msgText);
				{
					ChassisUpdateMessageHandler chssUpdtHandler = new ChassisUpdateMessageHandler("com.matson.tos.jaxb.snx",
							"com.matson.tos.jatb", "/xml/cupd.xml", AbstractMessageHandler.TEXT_TO_XML);
					if (chssUpdtHandler != null) {
						logger.debug("msgText before post:" + msgText);
						chssUpdtHandler.setTextStr(msgText);
						logger.debug("chssUpdtHandler " + chssUpdtHandler.getTextStr());
						xml = chssUpdtHandler.getXmlStr();
						logger.debug("xml before post:" + xml);
						JMSSender sender1 = new JMSSender(JMSSender.REAL_TIME_QUEUE, chssUpdtHandler.getFacilityToPost());
						logger.info("Posting Chassis update message to : " + chssUpdtHandler.getFacilityToPost());

						if (xml != null) {
							logger.debug("chassis update XML for MQ :" + xml);
							sender1.send(xml);
						} else {
							logger.warn("Unit SNX with chassis update not sent to TOS, XML is " + xml);
						}
					}
				}
			...
			if (msgHandler != null/* && !msgText.startsWith("ABD")*/) {
				msgHandler.setTextStr( msgText);				
				xml = msgHandler.getXmlStr();
				/*logger.debug( "XML:" + xml);
				JMSSender sender = new JMSSender( JMSSender.REAL_TIME_QUEUE, "ANK");
				...
				if(msgText.startsWith( "ABDA") || msgText.startsWith( "ABDB")){
					messageinQueue = isMessageQueued(msgText,xml);
				}//Msg check IF 
				if(msgText.startsWith( "ALNK")){
					messageinQueue = isMessageQueued(msgText,xml);
					if(!messageinQueue){
					TosLookup unitLookUp = new TosLookup();
					String vesvoy = getFieldValue(xml, "vesvoy");
					String unitFacility = unitLookUp.getUnitActiveFacilityinAlaska(getUnitNbr(xml),vesvoy);
					JMSSender sender=null;
					logger.info("unitFacility: "+unitFacility);
					//if(unitFacility!=null){
					if(("KDK").equalsIgnoreCase(unitFacility.trim())){
						 logger.debug( "in KDK " + unitFacility+" ");
						  sender = new JMSSender( JMSSender.REAL_TIME_QUEUE, "KDK");
					 }
					 else if(("DUT").equalsIgnoreCase(unitFacility.trim())||("KQA").equalsIgnoreCase(unitFacility.trim())||("SDP").equalsIgnoreCase(unitFacility.trim())){
						 logger.debug( "in DUT " + unitFacility+" ");
						  sender = new JMSSender( JMSSender.REAL_TIME_QUEUE, "DUT");
					 }
					 else {
						 logger.debug( "in ANK " + unitFacility+" ");
						  sender = new JMSSender( JMSSender.REAL_TIME_QUEUE, "ANK");
					 }
					logger.info(xml);
					 sender.send( xml);
					 messageinQueue=true;
				...
				//D032475 start
				if(msgText.startsWith("ACAR")||msgText.startsWith("AHLP")||msgText.startsWith("AHLR")||msgText.startsWith("AARD")){
					
					String port=null;
					
					if(msgText.startsWith("ACAR")){
						
						port= ((Acar)msgHandler.getTextObj()).getLoadPort();
						 logger.debug( "port1: " + port+" ");
					}
					else if(msgText.startsWith("AHLP")||msgText.startsWith("AHLR")){
						
						port= ((Ahlx)msgHandler.getTextObj()).getHoldPort();
						 logger.debug( "port2: " + port+" ");
					}
					else{
						
						port= ((Aard)msgHandler.getTextObj()).getArdEqlgLoadPort();
						 logger.debug( "port3: " + port+" ");
					}
					 ...
					 if(("KDK").equalsIgnoreCase(port.trim())){
						 
						 logger.debug( "in KDK " + port+" ");
						 
						  sender = new JMSSender( JMSSender.REAL_TIME_QUEUE, "KDK");	 
					 }
					 else if(("DUT").equalsIgnoreCase(port.trim())||("KQA").equalsIgnoreCase(port.trim())||("SDP").equalsIgnoreCase(port.trim())){
						 
						 logger.debug( "in DUT " + port+" ");
						 
						  sender = new JMSSender( JMSSender.REAL_TIME_QUEUE, "DUT");
					 }
					 else {
						 
						 logger.debug( "in ANK " + port+" ");
	 ...
	 public boolean isMessageQueued(String msgText,String xml)
	 {
	   ...
	   try
	   {
		...
		logger.info("vesvoy : "+vesvoy);
		   if(vesvoy != null && !(vesvoy.startsWith("M"))){		    
		 	isBarge = true;
		}else{	
		  ...
		  String unitNbr = getUnitNbr(xml);
		   try{		
		    category = unitNbr != null && unitNbr.length() > 0 ?
		          lookup.getActiveUnitCategory(unitNbr, "ANK") : null;
		          
		          if (category == null){
		        	  category = unitNbr != null && unitNbr.length() > 0 ?
		    		          lookup.getActiveUnitCategory(unitNbr, "KDK") : null;
		          }
		          if (category == null){
		        	  category = unitNbr != null && unitNbr.length() > 0 ?
		    		          lookup.getActiveUnitCategory(unitNbr, "DUT") : null;
		          }
